...
Cylinder = [cyPoly12, cyPoly23, cyPoly34, cyPoly45, cyPoly56, cyPoly67, cyPoly78, cyPoly81, ['x', 'x', 'x']]
...
def drawScene(scene):
    ...
    for index in range(len(scene)):
        if(index == shape):
            ...
            color = "blue"
        drawPoly(scene[index], color, zbuffer)
...
def drawPoly(poly, lcolor, zbuffer):
 ...
 ############Intensity1 and Intensity2 need to be determined before here#################################
            ...
            normalL = [1, 1, -1]
            #I get to define kd and ks and n values (but n should be big enough for noticeable shine)
            kd = [180, 150, 0]
            kg = [180, 150, 0]
            ...
            for x in range(int(x1), int(x2), step):
                ...
                Ip = [1 - (abs(x-lightLoc[0])/lightLoc[0]), 1 - (abs(x-lightLoc[0])/lightLoc[0]), 1 - (abs(x-lightLoc[0])/lightLoc[0])]
