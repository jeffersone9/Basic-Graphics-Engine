...
def checkerboard_point_intensity(x, y, z, ir, ig, ib):
...
def sphere_intersection(xs, ys, zs, ray_x, ray_y, ray_z, t, intersect_x, intersect_y, intersect_z, obj_normal_x, obj_normal_y, obj_normal_z, sphere):
...
def sphere_point_intensity(level, ray_x, ray_y, ray_z, x, y, z, nx, ny, nz, ir, ig, ib):
...
def trace_ray(flag, level, xs, ys, zs, ray_i, ray_j, ray_k, ir, ig, ib):
    if(level == 0): #maximum depth, which means the color is black
        ...
        object_code = -1
        if(checkerboard_intersection(xs, ys, zs, ray_i, ray_j, ray_k, t, intersect_x,
        intersect_y, intersect_z)):
            object_code = 0
            if(flag):
                print("checkerboard")
        for i in range(len(Scene)):
            if(sphere_intersection(xs, ys, zs, ray_i, ray_j, ray_k, t, intersect_x,
            intersect_y, intersect_z, obj_normal_x, obj_normal_y, obj_normal_z, Scene[i])):
        ...
        if(object_code == 0):
            checkerboard_point_intensity(intersect_x, intersect_y, intersect_z, ir, ig, ib)
        elif(object_code == 1):
            sphere_point_intensity(level, ray_i, ray_j, ray_k, intersect_x,
            intersect_y, intersect_z, obj_normal_x, obj_normal_y, obj_normal_z, ir, ig, ib)
        elif(object_code == 2):
            sphere_point_intensity(level, ray_i, ray_j, ray_k, intersect_x,
            intersect_y, intersect_z, obj_normal_x, obj_normal_y, obj_normal_z, ir, ig, ib)
        else:
...
def render():
    ...
    zs = -800
    #paint screen
    for pixel_x in range(1, CanvasWidth):
        screen_x = pixel_x - (CanvasWidth/2)
        for pixel_y in range(1, CanvasHeight):
